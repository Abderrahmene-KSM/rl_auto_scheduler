...
class SchedulerService:
    ...
    def apply_reversal(self, action):
        ...
        for comp in action.comps:
            ...
            self.schedule_object.comps_trans[comp]["matrix"] = np.dot(self.schedule_object.comps_trans[comp]["matrix"], get_trasnformation_matrix_from_vector(transformation,self.schedule_object.comps_trans[comp]["nb_it"]))
            # mark the computation as transformed
            self.schedule_object.comps_trans[comp]["transformed"] = True
            for branch in self.branches:
                # Check for the branches that needs to be updated
                if comp in branch.comps:
                    # Update the schedule
    ...
    def apply_addingOne(self, action):
        for comp in action.comps:
            row = self.schedule_object.comps_trans[comp]["row_number"]
            col = self.schedule_object.comps_trans[comp]["col_number"]
            matrix = self.schedule_object.comps_trans[comp]["matrix"]
    ...
    def apply_nextRow(self, action):
        for comp in action.comps:
            row = self.schedule_object.comps_trans[comp]["row_number"]
            nb_it = self.schedule_object.comps_trans[comp]["nb_it"]
            self.schedule_object.comps_trans[comp]["row_number"] = (row +1)% (nb_it-1)
            col = self.schedule_object.comps_trans[comp]["col_number"]
            row = self.schedule_object.comps_trans[comp]["row_number"]
            if (col == row):
                self.schedule_object.comps_trans[comp]["col_number"]= col +1
    ...
    def apply_nextCol(self, action):
        for comp in action.comps:
            ...
            row = self.schedule_object.comps_trans[comp]["row_number"]
            nb_it = self.schedule_object.comps_trans[comp]["nb_it"]
            self.schedule_object.comps_trans[comp]["col_number"] = (col +1)% nb_it + row +1 
